...
const ClimateAssessmentFloatingButton: React.FC<ClimateAssessmentFloatingButtonProps> = ({ isVisible }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <button
        onClick={toggleModal}
        className={`w-16 h-16 rounded-full shadow-2xl hover:shadow-3xl transition-all duration-300 flex items-center justify-center transform hover:scale-110 active:scale-95 border-2 border-white/20 backdrop-blur-sm ${
          isSubmitted 
      ...
      {/* Popup de Avaliação */}
      {showModal && (
        <div className="absolute bottom-20 right-0 w-96 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden backdrop-blur-sm"
          style={{
        ...
        >
          ...
          <div className="p-4 max-h-96 overflow-y-auto bg-gradient-to-b from-white to-gray-50/30">
            {isLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Avaliação Enviada!</h3>
                <p className="text-sm text-gray-600 mb-4">Sua avaliação de clima organizacional foi submetida com sucesso.</p>
              ...
              <form className="space-y-4" onSubmit={e => { e.preventDefault(); handleSubmit(); }}>
                  ...
                  return (
                    <div key={criterion.id} className="space-y-2 p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-gray-800 text-sm">{criterion.label}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 mb-2">
                        {[1,2,3,4,5].map(star => (
                          ...
                          >
                            <Star
                              className={`w-6 h-6 transition-all duration-200 ${(formData as any)[scoreKey] >= star ? 'text-yellow-400 fill-yellow-300 drop-shadow-sm' : 'text-gray-300 hover:text-yellow-200'}`}
                      ...
                      <textarea
                        ...
                        className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-400 focus:border-transparent resize-none text-xs bg-white shadow-sm transition-all duration-200"
                        rows={2}
                        required
                      />
                    </div>
                ...
                <div className="flex justify-between pt-2">
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 disabled:opacity-50 transition-all duration-200 font-medium text-sm"
                  >
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="px-6 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 disabled:opacity-50 transition-all duration-200 font-medium text-sm shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95"
                  >
