...
const PersonalInsightsCard: FC<PersonalInsightsCardProps> = ({ collaboratorId, cycle, onInsightsGenerated }) => {
  const toast = useGlobalToast();
  const { generateInsights, checkExistingInsights, clearInsights, insights, loading, checkingExisting, error } =
    usePersonalInsights();
        ...
        <div className='space-y-4'>
          ...
          <div className='bg-white rounded-lg p-4 border border-purple-100'>
            ...
            <div className='bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-4 border-l-4 border-purple-400'>
              <MarkdownRenderer content={insights.insights} className='text-gray-800' />
            </div>
          </div>
        </div>
      )}
      ...
      {!insights && !loading && !error && (
        <div className='bg-purple-50 border border-purple-200 rounded-lg p-4'>
          <div className='flex items-start gap-3'>
            ...
            <div>
              <h4 className='text-sm font-medium text-purple-900 mb-1'>💡 Insights ainda não gerados</h4>
              <p className='text-sm text-purple-800 mb-3'>
                Clique em "🚀 Gerar Insights" para criar uma análise personalizada das suas avaliações e receber dicas
                práticas para seu desenvolvimento.
              </p>
              <div className='bg-white rounded-lg p-3 border border-purple-100'>
                ...
                <ul className='text-sm text-purple-800 space-y-1 mt-2'>
                  <li>
                    • <strong>Seus pontos fortes</strong> identificados nas avaliações
                  </li>
                  <li>
                    • <strong>Oportunidades de crescimento</strong> personalizadas
                  </li>
                  <li>
                    • <strong>Padrões comportamentais</strong> observados pelos colegas
                  </li>
                  <li>
                    • <strong>Dicas práticas</strong> para melhorar sua performance
                  </li>
                  <li>
                    • <strong>Próximos passos</strong> para seu desenvolvimento
                  </li>
                </ul>
              </div>
            </div>
