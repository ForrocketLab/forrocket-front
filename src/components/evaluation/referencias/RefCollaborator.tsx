...
const RefCollaborator = () => {
  ...
  useEffect(() => {
    if (searchQuery.trim() !== '') {
      const addedIds = referenceFeedbackData.map(f => f.id);
      const results = allCollaborators.filter(
        c => !addedIds.includes(c.id) && c.name.toLowerCase().includes(searchQuery.toLowerCase()),
      );
        ...
        <div className='mb-6'>
          <div className='max-w mx-auto relative'>
            <h2 className='text-lg font-semibold mb-2'>
              Adicionar Referência ({referenceFeedbackData.length}/{totalEvaluations})
            </h2>
          ...
          {selectedCollaborator && (
            <form onSubmit={handleAddReference} className='block w-full mt-4 p-6 rounded-2xl bg-white shadow-lg'>
              ...
              <div className='mt-5 flex flex-col items-center'>
                ...
                <button
                  type='submit'
                  className='mt-4 bg-[#085F60] text-white font-medium text-sm py-2 px-6 rounded-sm hover:bg-[#086014d5]'
                >
                  Adicionar Feedback
                </button>
              </div>
          ...
          {referenceFeedbackData.length > 0 ? (
            referenceFeedbackData.map(feedback => (
              ...
              >
                ...
                <button
                  onClick={() => removeReferenceFeedback(feedback.id)}
