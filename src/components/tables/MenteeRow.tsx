// src/components/tables/MenteeRow.tsx
...
export interface MenteeRowProps {
  id: string;
  initials: string;
  name: string;
  jobTitle: string;
...
const MenteeRow: FC<MenteeRowProps> = ({
  id,
  initials,
  name,
  jobTitle,
  ...
  const handleNavigateToDetails = () => {
    if (!id) {
      console.error('ID do mentorado não definido!');
      return;
    }
    navigate(`/mentor/mentees/${id}/evaluations`);
  };

  return (
    <div className='px-6 py-4 border-b border-gray-200 hover:bg-gray-50 transition-colors'>
      <div className='grid grid-cols-12 gap-4 items-center'>
        {/* Mentorado - col-span-4 */}
