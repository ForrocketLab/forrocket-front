...
interface MenteesTableWithPaginationProps {
  mentees: DashboardSubordinate[];
  currentPage?: number;
  itemsPerPage?: number;
...
const MenteesTableWithPagination: FC<MenteesTableWithPaginationProps> = ({
  mentees,
  currentPage = 1,
  itemsPerPage = 10,
  sortBy = 'name',
  ...
  const processedData = useMemo(() => {
    ...
    if (actualFilterStatus !== 'ALL') {
      filteredMentees = filteredMentees.filter(mentee => mentee.assessmentStatus === actualFilterStatus);
    }
    ...
    if (actualSearchTerm) {
      filteredMentees = filteredMentees.filter(
        mentee =>
          (mentee.name || '').toLowerCase().includes(actualSearchTerm.toLowerCase()) ||
          (mentee.jobTitle || '').toLowerCase().includes(actualSearchTerm.toLowerCase()),
      );
    }
    ...
    const paginatedData = sortedMentees.slice(startIndex, endIndex);

    const paginationMeta = {
      page: actualCurrentPage,
      limit: itemsPerPage,
      total: sortedMentees.length,
      totalPages: Math.ceil(sortedMentees.length / itemsPerPage),
      hasNext: endIndex < sortedMentees.length,
    ...
    return {
      mentees: paginatedData,
      paginationMeta,
    };
  }, [mentees, actualCurrentPage, actualSortBy, actualSortOrder, actualSearchTerm, actualFilterStatus, itemsPerPage]);
    ...
    <div className='bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden relative'>
      ...
      <div className='px-6 py-4 border-b border-gray-200'>
        <div className='flex justify-between items-center'>
          <h2 className='text-lg font-semibold text-gray-800'>Mentorados</h2>
      ...
      {processedData.mentees.length === 0 && !isLoading ? (
        ...
        <>
          <div className='overflow-x-auto'>
            ...
            {/* Container com altura dinâmica baseada no número de mentorados */}
            <div className='bg-white'>
              {/* Linhas de mentorados */}
              {processedData.mentees.map(mentee => (
                <MenteeRow key={mentee.id} {...mentee} />
              ))}
            </div>
          </div>
            ...
            <div className='px-6 py-4 flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 gap-4'>
              ...
              <div className='flex items-center space-x-2 text-sm text-gray-700'>
                <span>
                  ...
                  de {processedData.paginationMeta.total} mentorados
                </span>
              </div>
...
export default MenteesTableWithPagination;
