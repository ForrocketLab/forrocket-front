        ...
        {ALLOWED_CRITERIA_IDS.map((criterionId, index) => {
          ...
          return (
            <div key={criterionId}>
              ...
              >
                <div className='flex items-center gap-3'>
                  {hasManagerAssessment ? <CheckCircle className='w-5 h-5 text-green-500 flex-shrink-0' /> : <div className='w-5 h-5 flex-shrink-0 flex items-center justify-center rounded-full border border-gray-300 text-xs font-medium text-gray-600'>{index + 1}</div>}
                  <h3 className='font-medium text-gray-900'>{getCriterionName(criterionId)}</h3>
                </div>
                <div className='flex items-center gap-4'>
                  <div className='text-sm font-medium text-gray-600'>{selfAnswer ? selfAnswer.score.toFixed(1) : '-'}</div>
                  <div className='text-sm font-medium text-gray-900'>{managerScore > 0 ? managerScore.toFixed(1) : '-'}</div>
                  ...
                  <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4'>
                    <div className='space-y-3'>
                      ...
                      <div className='flex items-center gap-1'>
                        {[1, 2, 3, 4, 5].map(starValue => <Star key={starValue} className={`w-5 h-5 ${starValue <= (selfAnswer?.score ?? 0) ? 'text-teal-500 fill-current' : 'text-gray-300'}`} />)}
                      </div>
                      <div>
                        <div className='text-sm font-medium text-gray-700 mb-2'>Justificativa</div>
                        <div className='bg-white p-3 rounded-md border border-gray-200 text-sm text-gray-700'>{selfAnswer?.justification || 'Nenhuma justificativa fornecida'}</div>
                      </div>
                    </div>
                    <div className='space-y-3'>
                      <div className='text-sm font-medium text-gray-700'>Sua avaliação de 1 a 5 com base no critério</div>
                      <div className='flex items-center gap-1'>
                        {[1, 2, 3, 4, 5].map(starValue => (
                          <button key={starValue} type='button' onClick={() => onRatingChange(criterionId, starValue)} className={`transition-colors hover:scale-110 ${isAssessmentSubmitted ? 'cursor-not-allowed opacity-50' : ''}`} disabled={isAssessmentSubmitted}>
                            <Star className={`w-5 h-5 ${starValue <= managerScore ? 'text-teal-600 fill-current' : 'text-gray-300 hover:text-teal-400'}`} />
                          </button>
                        ))}
                      </div>
                      <div>
                        <div className='text-sm font-medium text-gray-700 mb-2'>Justifique sua nota</div>
                        <textarea rows={4} placeholder='Justifique sua nota...' value={managerAssessments[criterionId]?.justification || ''} onChange={e => onJustificationChange(criterionId, e.target.value)} className={`w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-sm resize-none placeholder-gray-400 ${isAssessmentSubmitted ? 'bg-gray-100 cursor-not-allowed' : ''}`} disabled={isAssessmentSubmitted} />
                      </div>
                    </div>
                  </div>
                </div>
